...
first (in parallel, nested as deep as you like); the parent is parked in the
`waiting-children` state and runs only once every child has settled. One
...
parent = await queue.add_flow(
    ...
    attempts=3,                       # parent options: same kwargs as add()
)
...
`FlowChild` takes the same options as `Queue.add()` (`priority`, `attempts`,
`backoff`, `delay`, auto-removal), plus `on_fail` (below). A few options are
...
stalled-recovery limit settles its parent exactly like a normal failure.
...
- Removing a **parent** removes its whole subtree - children included, even
  ones currently running. This is how you cancel a flow.
...
level. `Job.parent_id` and `Job.children_ids` expose flow membership on any
loaded job. [matador](https://github.com/ilovepixelart/matador) renders all of
