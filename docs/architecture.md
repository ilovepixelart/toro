...
- **Retention** - `remove_on_complete` / `remove_on_fail` (bool / count /
  ...
  A flow is retained as one unit: a running flow's finished children score above
  the trims' reach, and a trimmed root takes its subtree. See
  [Producing jobs](producing.md#retention) and [Flows](flows.md).
